...
for i in range(trainEpoch):
	for i in range(trainEpoch):
			for jj in range(40):
				...
				for j in range(numIter):
								...
								if i%5 == 4:
												...
												writeWav(reconstructed.astype(np.float32), filename = "wavdumps_with/aud_re"+str(j)+".wav")
												...
												reconstructed = reconstructed.reshape((1, data.shape[2]*4, data.shape[3]*4, data.shape[1]))
												reconstructed[0,:,:,1] = ugdata[j*bs,:,:,1]
												writeWav(reconstructed.astype(np.float32), filename = "wavdumps_with/im_re"+str(j)+".wav")
												...
												reconstructed = reconstructed.reshape((1, data.shape[2]*4, data.shape[3]*4, data.shape[1]))
												reconstructed[0,:,:,1] = ugdata[j*bs,:,:,1]
												writeWav(reconstructed.astype(np.float32), filename = "wavdumps_with/true"+str(j)+".wav")


								#print(currentData.shape, reconstructedMean.shape)
								print_losses[0] += losses[0].item()*weight[0]
								print_losses[1] += losses[1].item()*weight[1]
								print_losses[2] += losses[2].item()*weight[2]
			...
			if i %dropLr == dropLr - 1:
					...
					LR['Generator']/= dropBy
			if True:
					torch.save(model, 'modelwith.pth')
