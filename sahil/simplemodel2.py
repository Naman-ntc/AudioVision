...
def vae_loss(mean, sigma, outMean, outSigma, target, index, weight=1):
	...
	target_loss = F.mse_loss(outMean[:,:,:,:], target[:,:,:,:])
...
class AudioEncoder(nn.Module):
		...
		def __init__(self, latentDim=128, encChannels=256, specRatio=(3,8), specShape=(192,512), leakyRelu=True):
				...
				self.initblock = nn.Conv2d(1, encChannels//16, kernel_size=1, groups=ngroups)
				self.blocks = nn.ModuleList([
						A.EncDiscBlock(encChannels//16, encChannels//16, leakyRelu, False),
...
class AudioGenerator(nn.Module):
		...
		def __init__(self, latentDim=128, genChannels=256, specRatio=(3,8), specShape=(192,512), leakyRelu=True, pixelNorm=False, normalize_latents=False, useSoftPlus = True):
				...
				self.endblockmean = nn.Conv2d(genChannels//16, 1, kernel_size=1, groups=ngroups)
				self.endblocklogsig = nn.Conv2d(genChannels//16, 1, kernel_size=1, groups=ngroups)
